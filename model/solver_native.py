    ...
    def validation(self, epoch):
        if not self.cfg.train.test_time:
            ...
            for i in range(val_data.shape[0]):
                ...
                if self.cfg.system.save_nii:
                    common_file.save_nii(tmp_label, os.path.join(self.val_dir, "%d_label.nii.gz" % i))
                    common_file.save_nii(tmp_pred, os.path.join(self.val_dir, "%d_pred.nii.gz" % i))

            self.val_dice.append(dsc_list.mean())

            tmp_val_score = dsc_list.mean() * 1 - self.tgt_pse_seg_loss[-1]*0.5
